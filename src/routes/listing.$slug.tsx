      ...
      <div className="mt-4 grid items-start gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-2">
            ...
            <Badge variant="outline">{difficultyLabel(listing.difficulty)}</Badge>
          ...
          <p className="mt-2 flex items-center gap-2 text-sm text-muted">
            ...
            <span>· {listing.reviewCount} ocena</span>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {[
              { icon: Clock, label: "Trajanje", value: listing.duration },
              { icon: Users, label: "Grupa", value: listing.groupSize },
              { icon: MapPin, label: "Sastanak", value: listing.meetingPoint },
            ].map((item) => (
              <div
                key={item.label}
          ...
          <h2 className="mt-10 font-display text-2xl font-medium tracking-tight">U ceni</h2>
          ...
          <h2 className="mt-10 font-display text-2xl font-medium tracking-tight">Tok ture</h2>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted">
                {listing.hostRole} · {listing.hostYears} god. na terenu
              </p>
              <a
                href={`tel:${tel}`}
          ...
          {listing.reviews.length > 0 && (
            <>
              <h2 className="mt-10 font-display text-2xl font-medium tracking-tight">Utisci</h2>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-bg/95 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] backdrop-blur-md lg:hidden">
        <div className="mx-auto flex max-w-6xl items-center gap-3">
          <div className="min-w-0 flex-1">
            <p className="font-medium tabular-nums">{formatRsd(listing.priceRsd)}</p>
            <p className="truncate text-xs text-muted">/ {priceUnitLabel(listing.priceUnit)}</p>
          </div>
          <SaveButton slug={listing.slug} title={listing.title} variant="outline" iconOnly className="shrink-0" />
