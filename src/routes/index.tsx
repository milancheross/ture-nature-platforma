...
import { CATEGORIES, CATEGORY_IMAGE, REGIONS } from "@/lib/catalog";
import { catBlurb, catLabel, useI18n } from "@/lib/i18n";
...
function Home() {
  ...
  const { t } = useI18n();
  const howIcons = [Compass, Handshake, MapPinned];
  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
          {CATEGORIES.map((cat) => (
            ...
            >
              <img
                src={listingImage(CATEGORY_IMAGE[cat.id])}
                alt=""
                className="aspect-[4/3] w-full rounded-lg object-cover outline outline-1 -outline-offset-1 outline-fg/10"
              />
              <div className="px-2.5 py-3">
                <p className="text-sm font-medium text-fg">{catLabel(t, cat.id)}</p>
                <p className="text-xs text-muted">{catBlurb(t, cat.id)}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        <h2 className="font-display text-3xl font-medium tracking-tight">{t.home.regionsTitle}</h2>
        <p className="mt-2 max-w-lg text-sm text-muted">{t.home.regionsSubtitle}</p>
